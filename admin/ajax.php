...
if(!empty($_POST["tablo"]) && !empty($_POST["ID"]) && $_FILES && !empty($_FILES["file"]["tmp_name"]))
	{
		 ...
		 $tablo=$VT->filter($_POST["tablo"]);
   		 ...
   		 $resim=$VT->uploadMulti("file",$tablo,$ID,"assets/images/resimler/");
}
						...
						<?php 
							for($i=0;$i<count($_POST["secenek1"]); $i++)
							{
								...
								?>
									...
									<td><input type="number" name="stok[]" value="1" min="1"></td>
								<?php
								}
								echo '</tr>';	
			...
			<table class="table">
					...
					<?php 
						for($i=0;$i<count($_POST["secenek1"]); $i++)
						{
							...
							<td><input class="form-control" type="number" name="stok[]" value="1" min="1"></td>
							</tr>
							<?php
						}
					 ?>
