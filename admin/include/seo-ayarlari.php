...
$veri = $VT->VeriGetir("ayarlar", "WHERE ID=?", array(1), "ORDER BY ID ASC", 1);
?>
      ...
      if ($_POST) {
        if (!empty($_POST["baslik"]) && !empty($_POST["anahtar"]) && !empty($_POST["description"])) {
          ...
          if (!empty($_FILES["resim"]["name"])) {
            $yukle = $VT->upload("resim", "assets/images/siteayarlari/");
            if ($yukle != false) {
              $guncelle = $VT->SorguCalistir("UPDATE ayarlar", "SET baslik=?, anahtar=?, aciklama=?, image=? WHERE ID=?", array($baslik, $anahtar, $description, $yukle, 1), 1);
            } else {
              $ekle = false;
      ?>
          ...
          if ($guncelle != false) {
            ...
            <meta http-equiv="refresh" content="1;url=<?= SITE ?>seo-ayarlari" />
          <?php
          } else {
          ?>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  <label for="imageinput">Profil Fotoğrafı
                    <?php
                    if ($veri[0]["image"] == NULL) {
                      $varsayilan = "varsayilanimage.png";
                    } else {
                      $varsayilan = $veri[0]["image"];
                    }
                    ?>
                    <img src="<?= SITE ?>assets/images/siteayarlari/<?= $varsayilan ?>" alt="" width="80" height="80" style="margin-left: 80px;">
                  </label>
