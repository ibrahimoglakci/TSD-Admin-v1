...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Ürünler Listesi</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
				...
				if($veriler!=false)
				{
					...
					for($i=0;$i<count($veriler);$i++)
					{
                        ...
                        <tr>
                          <td><?=$sira?></td>
                          <td>
                            <img src="<?=SITE?>assets/images/urunler/<?=$veriler[$i]["resim"]?>" style="height: 60px; width: auto; margin-right: 8px; float: left;">
                      ...
                      <label class="custom-control-label" for="customSwitch3<?=$veriler[$i]["ID"]?>"></label>
                          ...
                          <td>
                          <a href="<?=SITE?>urun-duzenle/<?=$veriler[$i]["ID"]?>" class="btn btn-warning btn-sm"><i class="fas fa-pen"></i> Düzenle</a>
                          <a href="<?=SITE?>urun-stok/<?=$veriler[$i]["ID"]?>" class="btn btn-primary btn-sm"><i class="fas fa-cog"></i> Stok Yönetimi</a>
                          <a href="<?=SITE?>resimler/urunler/<?=$veriler[$i]["ID"]?>" class="btn btn-success btn-sm" style="margin-top: 2px; margin-bottom: 2px;"><i class="fas fa-cog"></i> Resim Yönetimi</a>
                          <a href="<?=SITE?>urun-sil/<?=$veriler[$i]["ID"]?>" class="btn btn-danger btn-sm silmeAlani"><i class="fas fa-times-circle"></i> Kaldır</a>
                          </td>
                        </tr>
                        <?php
					}
