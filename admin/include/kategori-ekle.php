      ...
      <a href="<?=SITE?>kategori-liste" class="btn btn-info" style="float:right; margin-bottom:10px; margin-left:10px;"><i class="fas fa-bars"></i> LİSTE</a> 
	   ...
	   if($_POST)
	   {
		   if(!empty($_POST["kategori"]) && !empty($_POST["baslik"]) && !empty($_POST["anahtar"]) && !empty($_POST["description"]) && !empty($_POST["sirano"]))
		   {
			   ...
			   {
				   $yukle=$VT->upload("resim","assets/images/kategoriler/");
				   if($yukle!=false)
				   {
       ...
       <div class="card-body card card-primary">
            ...
            <div class="col-md-12">
                <div class="form-group">
                <label>Sıra No</label>
                <input type="number" class="form-control" placeholder="Sıra No ..." name="sirano" style="width:100px;" value="<?php $sirano=$VT->IDGetir("kategoriler"); 
                if($sirano!=false)
                {
                  echo $sirano;
                }
